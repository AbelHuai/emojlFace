...
-(void)create{
    ...
    _sv.contentSize = CGSizeMake(IPHONE_WIDTH*5, self.frame.size.height-20);
...
{
    int index = scrollView.contentOffset.x/IPHONE_WIDTH;
    int mod   = fmod(scrollView.contentOffset.x,IPHONE_WIDTH);
    if( mod >= 160)
        index++;
...
- (void) setPage
{
	_sv.contentOffset = CGPointMake(IPHONE_WIDTH*_pc.currentPage, 0.0f);
  //  NSLog(@"setPage:%f,%d",_sv.contentOffset,_pc.currentPage);
    [_pc setNeedsDisplay];
}
